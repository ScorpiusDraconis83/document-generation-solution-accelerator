...
export const ChatHistory = React.memo(function ChatHistory({ 
  ...
  const handleRenameConversation = useCallback(async (conversationId: string, newTitle: string) => {
    try {
      ...
      // Rename failed — no action needed
    }
  }, []);
...
const ConversationItem = React.memo(function ConversationItem({ 
  ...
  const handleRenameConfirm = async () => {
    ...
    if (trimmedValue.length < MIN_NAME_LENGTH) {
      setRenameError(`Conversation name must be at least ${MIN_NAME_LENGTH} characters`);
      return;
    }
    if (trimmedValue.length > MAX_NAME_LENGTH) {
      setRenameError(`Conversation name cannot exceed ${MAX_NAME_LENGTH} characters`);
      return;
    }
        ...
        <DialogSurface>
            ...
            <DialogContent>
              <Input
                ...
                onChange={(e) => {
                  ...
                  } else if (newValue.trim().length < MIN_NAME_LENGTH) {
                    setRenameError(`Conversation name must be at least ${MIN_NAME_LENGTH} characters`);
                  } else if (!/[a-zA-Z0-9]/.test(newValue)) {
                    setRenameError('Conversation name must contain at least one letter or number');
                  } else if (newValue.length > MAX_NAME_LENGTH) {
                    setRenameError(`Conversation name cannot exceed ${MAX_NAME_LENGTH} characters`);
                  } else {
                    setRenameError('');
                  }
              ...
              >
                Maximum {MAX_NAME_LENGTH} characters ({renameValue.length}/{MAX_NAME_LENGTH})
              </Text>
              {renameError && (
                <Text 
                  size={200} 
          ...
          <DialogActions style={{ marginTop: '8px', paddingTop: '8px', paddingBottom: '8px' }}>
            ...
            <Button 
              ...
              disabled={renameValue.trim().length < MIN_NAME_LENGTH || !/[a-zA-Z0-9]/.test(renameValue) || renameValue.length > MAX_NAME_LENGTH}
            >
              Rename
            </Button>
          </DialogActions>
