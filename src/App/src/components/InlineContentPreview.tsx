...
import { AI_DISCLAIMER, PRODUCT_DEFAULTS } from '../utils/constants';
import { useWindowSize } from '../hooks/useWindowSize';
import { isContentFilterError, getErrorMessage, copyToClipboard } from '../utils/errorUtils';
import { downloadImageWithBanner } from '../utils/imageDownload';
...
export const InlineContentPreview = React.memo(function InlineContentPreview({ 
  content, 
  onRegenerate, 
  isLoading, 
  selectedProduct, 
  ...
  const handleDownloadImage = () => {
    ...
    const headline = selectedProduct?.product_name || text_content?.headline || 'Your Product';
    downloadImageWithBanner(image_content.image_url, headline, text_content?.tagline);
  };

  const productDisplayName = selectedProduct?.product_name || text_content?.headline || 'Your Content';
        ...
        <div style={{ 
          ...
          <div style={{ position: 'relative' }}>
            <img
              ...
              alt={image_content.alt_text || PRODUCT_DEFAULTS.fallbackAltText}
              style={{
                width: '100%',
                height: 'auto',
                display: 'block',
...
const ViolationCard = React.memo(function ViolationCard({ violation }: { violation: ComplianceViolation }) {
