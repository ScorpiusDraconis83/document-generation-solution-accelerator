...
import { AI_DISCLAIMER, BRIEF_FIELD_CONFIG, BRIEF_FIELD_KEYS, BRIEF_FIELD_LABELS } from '../utils/constants';
...
export const BriefReview = React.memo(function BriefReview({
  brief,
  onConfirm,
  onStartOver,
  isAwaitingResponse = false,
}: BriefReviewProps) {
  const populatedFields = BRIEF_FIELD_KEYS.filter(key => brief[key]?.trim()).length;
  const missingFields = BRIEF_FIELD_KEYS.filter(key => !brief[key]?.trim());

  // Filter to only populated fields for display
  const populatedDisplayFields = BRIEF_FIELD_CONFIG.filter(({ key }) => brief[key]?.trim());
