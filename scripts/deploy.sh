...
echo "  - Azure OpenAI (GPT image model for image generation - can be separate resource)"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    if [ -n "$PRINCIPAL_ID" ]; then
        ...
        echo ""
        echo "2. Azure OpenAI (GPT image model - if separate resource):"
        echo "   Role: Cognitive Services OpenAI User"
        echo "   az role assignment create --assignee $PRINCIPAL_ID \\"
        echo "     --role 'Cognitive Services OpenAI User' \\"
        echo "     --scope <IMAGE_RESOURCE_ID>"
        echo ""
